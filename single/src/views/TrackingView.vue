  ...
  <div class="tracking-view">
    ...
    <Modal v-model="showWheelPicker" :title="activeExerciseName">
      <div v-if="activeExerciseIndex >= 0" class="picker-content">
        <!-- User tabs (only shown when there is more than one user) -->
        <div v-if="authStore.users.length > 1" class="user-tabs">
          <button
        ...
        <!-- Switch to other user hint (only relevant with more than one user) -->
        <p v-if="authStore.users.length > 1" class="picker-hint">
...
import { flushQueue } from '../services/syncService.js'
